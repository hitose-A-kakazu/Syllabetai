...
class ClassRegistration():
    def __init__(self, root):
        ...
        ######画面遷移用フレーム
        self.select_move_frame = ttk.Frame(root, relief='solid', style='MyWidget.TFrame')   #メインフレーム
        self.select_move_frame.grid(row=0, column=0, sticky="nsew")

        ####学年・学期入力フレーム
        self.class_frame = ttk.Frame(self.select_move_frame, relief='solid', style='MyWidget.TFrame')
        self.class_frame.grid()  # row=0,column=0
        self.select_move_frame.rowconfigure(0, weight = 1)
        self.select_move_frame.columnconfigure(0, weight = 1)
        #見出し
        tk.Label(self.class_frame, text="履修登録を行う年の学年・学期を入力・送信してください",
        bg = 'SlateBlue4', fg = 'White').grid(row=0, column=0, columnspan=5, pady = 1)
        ...
        def classframe_func(text, lr, lc, clist,var):
            tk.Label(self.class_frame, text=text, bg = 'SlateBlue4', fg = 'White').grid(row=lr, column=lc)
            self.class_frame.rowconfigure(lr, weight = 1)
            self.class_frame.columnconfigure(lc, weight = 1)
            for rbindex in range(0, len(clist)):
                rdo1 = tk.Radiobutton(self.class_frame, value=clist[rbindex],
                                      variable=var, text=clist[rbindex], bg ='SlateBlue4')
                rdo1.grid(row=lr+1+rbindex, column=lc, padx = 1, pady = 1)
        ...
        def create_display_first_frame():
            ...
            ####講義選択画面における親フレーム
            self.display_first_frame = ttk.Frame(self.select_move_frame, relief='solid',style='MyWidget.TFrame')    #メインフレーム
            self.display_first_frame.grid(row = 0, column = 0)
            ...
            self.select_move_frame.rowconfigure(0, weight = 1)
            self.select_move_frame.columnconfigure(0, weight = 1)
            self.select_move_frame.rowconfigure(1, weight = 1)
            self.select_move_frame.columnconfigure(1, weight = 1)
        ...
        def topback_func():
            findex.grid_forget()
            select_subject = topframe_kenya.Topframe(root)
